...
const hostQuiz = async (req, res) => {
  try {
    ...
    // Strip out answers before saving to HostedQuiz
    ...
    return res
      .status(201)
      .json({ message: "Quiz hosted successfully", sessionId, hostedQuiz: hosted });
  } catch (err) {
...
const getHosted = async (req, res) => {
  try {
    // Only return active session for this quizId (or sessionId)
    const hosted = await HostedQuiz.findOne({
      quizId: req.params.quizId,
      active: true
    });
    if (!hosted) return res.status(404).json({ error: "No active hosted quiz found" });
    res.json(hosted);
  } catch (err) {
    console.error(err);
...
// POST /host/:sessionId/stop (owner only)
const stopHost = async (req, res) => {
  try {
    ...
    if (!hosted.hostId.equals(req.user._id)) return res.status(403).json({ error: "Forbidden" });
    hosted.active = false;
    await hosted.save();
